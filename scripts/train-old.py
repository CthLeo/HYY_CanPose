...
class DopeNetwork(nn.Module):
    def __init__(
        ...
        # print('---Dimval----------------------------------------------')
        # _3 are the dim stages
        self.dim = DopeNetwork.create_dim(numBeliefMap, numDim)
    ...
    def create_dim(in_channels, out_channels):
        ...
        # Last convolution
        model.add_module("conv", nn.Conv2d(in_channels, out_channels, kernel_size=5, padding=0, stride=1))
...
def _runnetwork(epoch, loader, train=True):
    ...
    for batch_idx, targets in enumerate(loader):
        ...
        output_belief, output_affinities, output_dim = net(data)
        # print("belief.shape", len(output_belief), len(output_belief[0]),  len(output_belief[0][0]),  len(output_belief[0][0][0]),  len(output_belief[0][0][0][0]))
        # print("affinities.shape", len(output_affinities), len(output_affinities[0]))  
        # print("dim.shape", len(output_dim), len(output_dim[0]),  len(output_dim[0][0]),  len(output_dim[0][0][0]),  len(output_dim[0][0][0][0]))
        ...
        # for m in range(10):
        #     temp = target_belief[m, :, :, :]
        #     unloader = transforms.ToPILImage()
        #     myimage = temp.cpu().clone()
        #     myimage = myimage.squeeze(0)
        #     myimage = unloader(myimage)
        #     plt.imshow(myimage)
        #     plt.pause(1)
        ...
        # Dim loss
        for l in output_dim: #output, each belief map layers. 
            if loss_dim is None:
                loss_dim = ((l - target_dim) * (l-target_dim)).mean()
            else:
